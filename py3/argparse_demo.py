...
def main():
    parser = argparse.ArgumentParser(
        description="add, modify and delete upstream nodes"
    )
    parser.add_argument(
        "action",
        choices=["add", "modify", "delete"],
        help="action of upstream nodes, one of add, modify, delete",
    )
    parser.add_argument(
        "-n", "--name", required=True, type=str, help="upstream node name"
    )
    parser.add_argument("-l", "--loki_id", required=False, type=int, help="loki id")
    parser.add_argument("-p", "--port", required=False, type=int, help="port")
    parser.add_argument(
